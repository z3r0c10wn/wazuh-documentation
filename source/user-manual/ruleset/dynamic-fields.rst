...
.. note::
    By default, the number of fields that can be extracted simultaneously from an ``<order>`` tag is **256**. This value can be modified by changing the option :ref:`decoder_order_size <reference_ossec_analysis_decoder_order_size>` in ``/var/ossec/etc/ossec.conf``.
